...
func _ready() -> void:
	LogUtils.logs("maze dreaming started")
	rand.seed_random()
	Globals.generation_rand = rand
	dream_rooms()

func dream_rooms() -> void:
	LogUtils.logs("room dreaming started")
...
func decorate_rooms() -> void:
	LogUtils.logs("room decorating started")
...
func on_room_dreaming_finished(positions_: PackedVector2Array, shapes_: Array[Shape2D]) -> void:
	LogUtils.logs("room dreaming finished")
	positions = positions_
	shapes = shapes_
	decorate_rooms()

func on_room_decoration_finished() -> void:
	LogUtils.logs("room decorating finished")
	LogUtils.logs("maze dreaming finished")
